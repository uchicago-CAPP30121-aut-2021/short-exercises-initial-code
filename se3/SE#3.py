...
def find_candidates_from_city(candidates, office_loc):
    ...
    for candidate in candidates:
        if (candidate["City"], candidate["State"]) == office_loc:
            ret.append(candidate["Candidate_ID"])
    return ret
...
def construct_homestate_dict(candidates):
    ...
    ret = {}
    for candidate in candidates:
        ret[candidate["Candidate_ID"]] = candidate["State"]
    return ret
...
def find_successful_fund_raisers(cand_to_count, threshold):
    ...
    ret = []
    for k, v in cand_to_count.items():
        if v >= threshold:
            ret.append(k)
    return ret
...
def construct_cands_by_state(candidates):
    ...
    ret = {}
    for candidate in candidates:
        if candidate["State"] not in ret:
            ret[candidate["State"]] = [candidate]
        else:
            ret[candidate["State"]].append(candidate)
    return ret
